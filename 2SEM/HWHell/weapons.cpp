...
Weapons::Weapons(int width, int height) : width(width), height(height), player(0), isShooting(false), timeCount(0)
{
	angle[0] = 0;
	angle[1] = 180;
	speed[0] = 10;
	...
	QObject::connect(&shootTimer, SIGNAL(timeout()), this, SLOT(moveShoot()));	
}
...
void Weapons::moveShoot()
{
	...
	if (rect.intersects(players[player ^ 1]))
	{
		...
		restart(true);
		return;
	} else if (rect.x() > width || rect.y() > height || rect.x() < 0 || rect.y() < 0)
	{
		isShooting = false;
		shootTimer.stop();
	}
...
void Weapons::restart(bool fl)
{
	...
	speed[1] = 10;
	QMessageBox msb;
	msb.setText("Player" + QString::number(player) + "is win!");
	update();	
}
...
QRect Weapons::shotRect() const
{
	...
	y0 = y0 + (gun().height() + 5) * sin(radians);
